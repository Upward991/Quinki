...
- Task da chat (creazione da Calendar o tool `schedule_task`), run sincrono con stato
...
- OpenAI: OAuth subscription (ChatGPT Plus/Pro), GPT-5.x models
- Anthropic: OAuth subscription (Claude Pro/Max), Claude Opus/Sonnet/Haiku models
- GitHub Copilot: OAuth subscription ($10/mo), Claude + GPT models via Copilot
- xAI (Grok): OAuth subscription (SuperGrok/X Premium), Grok 4.x models
- cancel_schedule: tool per cancellare scheduled tasks (nella UI Agents e per gli agenti)
