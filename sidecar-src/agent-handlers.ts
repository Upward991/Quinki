...
export function createAgentHandlers(agentDir: string, getCwd: () => string) {
  ...
  return {
    ...
    updateAgent: async (p: any) => {
      ...
      // === EVENT-DRIVEN CONFIG REFRESH (29 ago — redesign): il send NON controlla
      // più i config a ogni messaggio (era: firma letta da disco per OGNI send +
      // dispose a sorpresa). Ora è il CAMBIAMENTO che notifica: le sessioni vive di
      // questo agente si rinfrescano in un momento SICURO (mai durante un turno).
      try { (globalThis as any).__quinki_piBridge?.refreshSessionsForAgent?.(String(p.id)); } catch {}
    ...
    // Blocco 5: legge il PROMPT.md di un agente (per il publish)
    readAgentPrompt: async (p: any) => {
      const id = String(p?.id || '').trim();
      if (!id) return { error: 'id required' };
      const f = path.join(agentDir, 'agents', id, 'PROMPT.md');
      try { return { content: fs.existsSync(f) ? fs.readFileSync(f, 'utf-8') : '' }; } catch { return { content: '' }; }
    },
    // Blocco 5: legge il config.json di un agente (per il publish)
    getAgentConfig: async (p: any) => {
      const id = String(p?.id || '').trim();
      if (!id) return { error: 'id required' };
      const f = path.join(agentDir, 'agents', id, 'config.json');
      try { return { content: fs.existsSync(f) ? fs.readFileSync(f, 'utf-8') : '' }; } catch { return { content: '' }; }
    },
    listChatSkills: async (p: any) => {
      // Returns skills grouped by agent, filtered by disable-model-invocation or user-invocable
      const agentIds: string[] = p.agentIds || [];
      if (agentIds.length === 0) return { groups: [] };
      const allSkills = scanSkills();
      const groups: any[] = [];
      for (const agentId of agentIds) {
        const cfg = readAgentConfig(agentId);
        if (!cfg?.skills || !Array.isArray(cfg.skills) || cfg.skills.length === 0) continue;
        const agentSkills = allSkills.filter((s: any) =>
          cfg.skills.includes(s.name) && (s.disableModelInvocation || s.userInvocable)
        );
        if (agentSkills.length > 0) {
          groups.push({
            agentId,
            agentName: cfg.name || agentId,
            skills: agentSkills.map((s: any) => ({ name: s.name, description: s.description || '' }))
          });
        }
      }
      return { groups };
    ...
    installSkill: async (p: any) => {
      ...
      try {
        ...
        fs.rmSync(tmpDir, { recursive: true, force: true });
        return { success: true, count: skillFiles.length, installed: installedNames };
      } catch (e: any) {
    ...
    deleteSkill: async (p: any) => {
      ...
      if (skillName === "app-expert" || skillName === "quinki-market") return { success: false, error: "Default skill cannot be deleted" };
    ...
    listTools: async () => ({
      tools: [
        ...
        { name: "cancel_schedule", description: "Cancel/delete a scheduled task by its ID (the user asks to stop a previously scheduled task)", readOnly: true },
        { name: "getTaskStatus", description: "List the status of all tasks of this session (scheduled + executed)", readOnly: true },
        { name: "getTaskResult", description: "Read the full result of a specific task execution by id", readOnly: true },
        { name: "readHandoff", description: "Read the handoff memory of this session's tasks", readOnly: true },
        { name: "screenshot", description: "Capture a screenshot of the Quinki app window (saved as PNG — the agent can read it to see the UI)", readOnly: true },
        { name: "market", description: "Use the Quinki Market: browse the catalog, search community items, install/uninstall packages and manage publishing", readOnly: true },
      ],
    }),
