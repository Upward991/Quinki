...
export class ExecutionEngine {
  ...
  messages(id: string): any[] {
    ...
    for (const e of entries) {
      ...
      if (role === "user") {
        ...
        const m: any = { id: e.id, role: "assistant", content: text, timestamp: ts, thinking, toolCalls, toolResults: [], agentModel: state?.model || undefined, thinkingLevel: state?.thinkingLevel || undefined, thinkingTranslated: state?.thinkingLevel || undefined, agentName: ((state?.agentIds || []).map((a: string) => a.split(/[-_]/).map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')).join(", ")) || undefined };
        out.push(m);
        lastAssistant = m;
  ...
  #appendHandoff(state: ExecutionState) {
    ...
    try {
      ...
      const record = [
        ...
        state.error ? `- Error: ${state.error}` : `- Note: ${state.progressNote || "ok"}`,
  ...
  async runTask(p: ExecutionParams): Promise<{ executionId: string; status: string }> {
    ...
    (async () => {
      ...
      try {
        ...
        // FIX (01 set): traduce il thinking level con il VERO livello del modello.
        // PRIMA: mostrava "on" generico. ORA: mostra "max"/"high"/etc. reale
        // (lo stesso che vedono le chat normali). Usa il metodo pubblico del PiBridge.
        try {
          const translated = (pb as any).translateThinkingForModel?.(state.thinkingLevel, state.model);
          if (translated && translated !== state.thinkingLevel) {
            this.#log("exec-thinking-translated", { executionId: id, from: state.thinkingLevel, to: translated, model: state.model });
            state.thinkingLevel = translated;
          }
        } catch {}
  ...
  async recover(autoResume = true): Promise<{ recovered: number; resumed: number }> {
    ...
    const now = Date.now();
    try {
      // === OWNER LOCK: ogni sidecar esegue SOLO i propri task (main | expert) ===
      // Senza questo filtro l'Expert riprendeva/rieseguiva anche i task della main
      // (stesso DB condiviso) → l'Expert "faceva girare" tutte le sessioni della main.
      const isExpertSidecar = (process.env.QUINKI_ROLE || "").toLowerCase() === "expert" || Number(process.env.QUINKI_WS_PORT || "9182") === 9183;
      const myOwner = isExpertSidecar ? "expert" : "main";
      for (const st of this.list()) {
        const stOwner = st.owner || "main";
        if (stOwner !== myOwner) { this.#log("recovery-skip-owner", { id: st.id, owner: stOwner, myOwner }); continue; }
