...
// PATCH (Quinki): BPE tokenizer — replaces the chars/4 heuristic with real token counts
// (gpt-tokenizer, cl100k_base). chars/4 overestimates code/JSON-heavy contexts and
// underestimates some prose — compaction must trigger at the right moment.
let _encoder = null;
...
export function estimateTokens(message) {
    // PATCH (Quinki): BPE-based estimates (see bpeTokens above).
    let tokens = 0;
    switch (message.role) {
        case "user": {
 ...
 */
function estimateTokens_chars4(message) {
    let chars = 0;
    switch (message.role) {
        case "user": {
