...
// === Patch: jiti/static non risolvibile in Electron ESM, use lazy dynamic import ===
let createJiti = null;
let _jitiInitialized = false;
async function ensureJiti() {
  if (_jitiInitialized) return;
  _jitiInitialized = true;
  try {
    const jitiStatic = await import("jiti/static");
    createJiti = jitiStatic.createJiti;
  } catch {
    try {
      const jitiMod = await import("jiti");
      createJiti = jitiMod.createJiti || jitiMod.default?.createJiti;
    } catch {}
  }
}
