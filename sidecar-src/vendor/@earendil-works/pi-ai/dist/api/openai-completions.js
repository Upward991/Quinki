...
function buildParams(model, context, options, compat = getCompat(model), cacheRetention = resolveCacheRetention(options?.cacheRetention, options?.env), grammarToolInputProperties = createGrammarToolInputProperties(context.tools, compat.supportsOpenAIGrammarTools)) {
    ...
    if (compat.thinkingFormat === "zai" && model.reasoning) {
        ...
        // PATCH Quinki (04 set): senza include_reasoning OpenRouter ragiona ma NON
        // restituisce il testo del reasoning → thinking invisibile all'agente.
        openRouterParams.include_reasoning = true;
        if (options?.reasoningEffort) {
            let eff = model.thinkingLevelMap?.[options.reasoningEffort] ?? options.reasoningEffort;
            // PATCH Quinki (04 set): caps AUTORITATIVE a livello richiesta (globalThis
            // alimentato dal bridge a ogni turno) — l'ultimo miglio non si bypassa mai.
            try {
                const capsStore = globalThis.__quinkiThinkingCaps;
                const capKey3 = (() => { try { return new URL(model.baseUrl).host + "/" + model.id; } catch { return model.id; } })();
                const capsMax = capsStore && capsStore[capKey3] ? capsStore[capKey3].maxLevel : null;
                if (capsMax) eff = capsMax;
            } catch {}
            openRouterParams.reasoning = { effort: eff };
        }
        else if (model.thinkingLevelMap?.off !== null) {
