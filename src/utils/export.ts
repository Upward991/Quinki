...
export function exportToMarkdown(messages: Message[], label: string): string {
  ...
  lines.push('')
  lines.push('> Full export — includes all messages, thinking, tool calls, delegations and metadata.')
  lines.push('')
  ...
  for (const m of sorted) {
    ...
    if (m.role === 'assistant') {
      ...
      if (m.thinking?.length) {
        for (const t of m.thinking) {
          if (t.text) { lines.push(toggle('Thinking', t.text)); lines.push('') }
        }
      }
      if (m.toolCalls?.length) {
        for (const tc of m.toolCalls) {
          const body = tc.args ? codeBlock('json', JSON.stringify(tc.args, null, 2)) : '(no args)'
          lines.push(toggle(`Tool call: ${tc.name}`, body)); lines.push('')
        }
      }
      if (m.toolResults?.length) {
        for (const tr of m.toolResults) {
          const lbl = tr.isError ? `Tool error: ${tr.name}` : `Tool result: ${tr.name}`
          lines.push(toggle(lbl, codeBlock('', tr.content || ''))); lines.push('')
        }
      }
      if (m.delegations?.length) {
        for (const d of m.delegations) {
          lines.push(`> **${d.agentName || 'Agent'} responded**`); lines.push('>')
          if (d.content) {
    ...
    if (m.role === 'system') {
      lines.push('---'); lines.push('')
      const lbl = m.isError ? '⚠️ Error' : 'System'
      lines.push(`**${lbl}**`); lines.push('')
      lines.push(m.content || ''); lines.push(''); continue
    }
  }
...
export function exportToHtml(messages: Message[], label: string): string {
  ...
  const body = sorted.map(m => {
    ...
    if (m.role === 'assistant') {
      ...
      if (m.thinking?.length) {
        for (const t of m.thinking) {
          if (t.text) content += `<details class="tool-call"><summary>Thinking</summary><div class="detail-content">${esc(t.text)}</div></details>`
        }
      }
      if (m.toolCalls?.length) {
        for (const tc of m.toolCalls) {
          content += `<details class="tool-call"><summary>Tool call: ${esc(tc.name)}</summary><div class="detail-content"><pre><code>${esc(JSON.stringify(tc.args, null, 2))}</code></pre></div></details>`
        }
      }
      if (m.toolResults?.length) {
        for (const tr of m.toolResults) {
          const cls = tr.isError ? 'tool-error' : 'tool-result'
          content += `<details class="${cls}"><summary>${tr.isError ? 'Tool error' : 'Tool result'}: ${esc(tr.name)}</summary><div class="detail-content"><pre><code>${esc(tr.content || '')}</code></pre></div></details>`
        }
      }
      ...
      return `<div class="msg msg-assistant"><div class="role-label role-agent">${esc(m.agentName || 'Assistant')}</div><div class="content">${content}</div>${footerHtml}</div>`
    }
    if (m.role === 'system') {
      const cls = m.isError ? 'msg-system' : 'msg-compaction'
      return `<div class="msg ${cls}"><div class="role-label ${m.isError ? 'role-system' : 'role-compaction'}">${m.isError ? 'Error' : 'System'}</div><div class="content">${esc(m.content || '')}</div></div>`
    }
    return ''
  }).join('\n')
...
<style>
:root { --bg:#1a1a2e; --surface:#16213e; --text:#e0e0e0; --text-secondary:#a0a0a0; --text-tertiary:#707070; --accent-info:#4a9eff; --danger:#e74c3c; --delegation:#c46a7e; --code-bg:#1e1e3e; --border:#2a2a4a; --user-bg:#1a1a3e; --radius:12px; --mono:'SF Mono','Fira Code',Consolas,monospace; --sans:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif; }
...
.footer { margin-top:8px; font-size:11px; color:var(--text-tertiary); display:flex; gap:6px; align-items:center; }
