...
function ViewRow({ v, active, renaming, renameVal, onRenameChange, onRenameCommit, onRenameCancel, onSelect, onContextMenu, multiSel, selected, onToggleSel }: { v: ViewCfg; active: boolean; renaming: boolean; renameVal: string; onRenameChange: (s: string) => void; onRenameCommit: () => void; onRenameCancel: () => void; onSelect: () => void; onContextMenu: (e: React.MouseEvent, v: ViewCfg) => void; multiSel: boolean; selected: boolean; onToggleSel: () => void }) {
  ...
  return React.createElement('div', { 'data-view-row-id': v.id, onMouseEnter: () => setH(true), onMouseLeave: () => setH(false), style: { padding: '2px 8px', opacity: isDragging ? 0.4 : 1 } }, [
...
export function CalendarView(props: { activePanel: string; onSelectPanel: (p: string) => void; agents?: any[]; onOpenSession?: (key: string) => void }) {
  ...
  const viewSidebar = React.createElement('div', { key: 'vsb', style: { position: 'absolute', top: 0, bottom: 0, left: 0, width: sideW, zIndex: 40, backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-floating)', display: 'flex', flexDirection: 'column', overflow: 'hidden' } }, [
    ...
    React.createElement('div', { key: 'list', style: { flex: 1, overflowY: 'auto', paddingBottom: 8, scrollbarGutter: 'stable' } }, React.createElement(DndContext, { key: 'dnd', sensors: useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } })), collisionDetection: closestCenter, onDragStart: () => { const rects = new Map<string, DOMRect>(); document.querySelectorAll('[data-view-row-id]').forEach(el => { const id = el.getAttribute('data-view-row-id'); if (id) rects.set(id, el.getBoundingClientRect()) }); itemRects.current = rects }, onDragMove: () => { const pointerY = pointerYRef.current; const ids = views.filter(v => v.id !== BASE_ID); for (const v of ids) { const rect = itemRects.current.get(v.id); if (!rect) continue; if (pointerY <= rect.bottom) { const zone = (pointerY < rect.top || (pointerY - rect.top) < rect.height / 2) ? 'before' : 'after'; setDropZone(prev => (prev && prev.id === v.id && prev.zone === zone) ? prev : { id: v.id, zone }); return } } if (ids.length) { const last = ids[ids.length - 1]; setDropZone(prev => (prev && prev.id === last.id && prev.zone === 'after') ? prev : { id: last.id, zone: 'after' }) } else setDropZone(null) }, onDragEnd: (e: any) => { setDropZone(null); const { active, over } = e; if (!over || active.id === over.id) return; const ids = views.filter(v => v.id !== BASE_ID); const from = ids.findIndex(v => v.id === active.id); const to = ids.findIndex(v => v.id === over.id); if (from === -1 || to === -1) return; persist([views[0], ...arrayMove(ids, from, to)]) }, onDragCancel: () => setDropZone(null) }, [
      React.createElement(SortableContext, { key: 'sc', items: views.filter(v => v.id !== BASE_ID).map(v => v.id), strategy: verticalListSortingStrategy }, (() => { const nodes: React.ReactNode[] = []; views.filter(v => v.id !== BASE_ID).forEach(v => { if (dropZone && dropZone.id === v.id && dropZone.zone === 'before') nodes.push(React.createElement('div', { key: 'indb_' + v.id, style: { height: 2, backgroundColor: 'var(--q-tab-accent)', margin: '0 8px', borderRadius: 1 } })); nodes.push(React.createElement(ViewRow, { key: v.id, v, active: v.id === activeId, renaming: renamingView === v.id, renameVal, onRenameChange: setRenameVal, onRenameCommit: () => renameView(v.id, renameVal), onRenameCancel: () => { setRenamingView(null); setRenameVal('') }, onSelect: () => setActiveId(v.id), onContextMenu: (e: any, vv: ViewCfg) => setCtxMenu({ x: e.clientX, y: e.clientY, view: vv }), multiSel, selected: selViews.has(v.id), onToggleSel: () => setSelViews(prev => { const n = new Set(prev); if (n.has(v.id)) n.delete(v.id); else n.add(v.id); return n }) })); if (dropZone && dropZone.id === v.id && dropZone.zone === 'after') nodes.push(React.createElement('div', { key: 'inda_' + v.id, style: { height: 2, backgroundColor: 'var(--q-tab-accent)', margin: '0 8px', borderRadius: 1 } })) }); return nodes })()),
    ])),
