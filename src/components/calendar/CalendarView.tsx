...
export function CalendarView(props: { activePanel: string; onSelectPanel: (p: string) => void; agents?: any[]; onOpenSession?: (key: string) => void }) {
  ...
  const tableFor = (rows: Item[]) => {
    ...
    const thead = React.createElement('tr', { key: 'thr' }, renderCols.map((k, idx) => { const c = COLS.find(x => x.key === k); const isLastCol = idx === renderCols.length - 1; if (k === 'sel') return React.createElement('th', { key: k, style: { padding: '9px 12px', minWidth: 52, borderBottom: '1px solid var(--q-border-solid)', borderRight: '1px solid var(--q-border-solid)' } }, React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 2 } }, [React.createElement(Checklist, { key: 'i', size: 15, style: { color: 'var(--q-text-tertiary)' } }), selTasks.size > 0 ? React.createElement(RowBtn, { key: 'bd', title: 'Delete ' + selTasks.size + ' selected', hoverColor: 'var(--q-accent-danger)', onClick: () => { const sels = filtered.filter(x => selTasks.has(x.id)); setDeleteTask({ items: sels.map(x => ({ id: x.id, kind: x.kind, title: x.title })) }) } }, React.createElement(Trash2, { size: 13 })) : null])); if (k === 'actions') return React.createElement('th', { key: k, style: { padding: '9px 12px', color: 'var(--q-text-secondary)', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font-interface)', whiteSpace: 'nowrap', userSelect: 'none', minWidth: COL_MIN.actions, borderBottom: '1px solid var(--q-border-solid)' } }, 'Actions'); return React.createElement('th', { key: k, onClick: () => toggleSort(k), style: { padding: '9px 12px', cursor: 'grab', color: 'var(--q-text-secondary)', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font-interface)', whiteSpace: 'nowrap', userSelect: 'none', minWidth: COL_MIN[k] || 80, borderBottom: '1px solid var(--q-border-solid)', borderRight: isLastCol ? 'none' : '1px solid var(--q-border-solid)' } }, c ? c.label : '') }))
  ...
  return React.createElement('div', { key: 'root', ref: rootRef, className: 'h-full flex flex-row', style: { width: '100%', position: 'relative' } }, [
    ...
    deleteTask ? React.createElement(ConfirmModal, { key: 'dt', title: deleteTask.items.length === 1 ? 'Delete task?' : 'Delete ' + deleteTask.items.length + ' tasks?', subtitle: deleteTask.items.length === 1 ? deleteTask.items[0].title + ' will be permanently deleted.' : deleteTask.items.length + ' tasks will be permanently deleted.', onCancel: () => setDeleteTask(null), onConfirm: () => doDeleteTask(deleteTask) }) : null,
  ])
}