...
function FilterChip({ label, values, options, onChange }: { label: string; values: string[]; options: string[]; onChange: (v: string[]) => void }) {
  ...
  const openMenu = () => {
    ...
    if (r) {
      ...
      if (r.bottom + menuH > window.innerHeight - 8) p.bottom = 'calc(100% + 4px)'; else p.top = 'calc(100% + 4px)'
    } else { p.left = 0; p.top = 'calc(100% + 4px)' }
    setPos(p)
  }
