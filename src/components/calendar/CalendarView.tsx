...
export function CalendarView(props: { activePanel: string; onSelectPanel: (p: string) => void; agents?: any[]; onOpenSession?: (key: string) => void }) {
  ...
  const cellStyle = (align = 'left', isLast = false, hover = false): React.CSSProperties => ({ padding: '9px 12px', color: 'var(--q-text-secondary)', fontSize: 13.5, fontFamily: 'var(--font-interface)', textAlign: align as any, whiteSpace: 'normal', wordBreak: 'break-word', backgroundColor: hover ? 'var(--q-hover)' : 'var(--q-bg)' })

  // Tabella per un gruppo (header colonne + righe) — la stessa struttura in ogni toggle
  const tableFor = (rows: Item[]) => {
    const thead = React.createElement('tr', { key: 'thr' }, cols.map((k, idx) => { const c = COLS.find(x => x.key === k); return React.createElement('th', { key: k, draggable: true, onDragStart: (e: any) => { setDragCol(k); e.dataTransfer.effectAllowed = 'move' }, onDragOver: (e: any) => e.preventDefault(), onDrop: (e: any) => { e.preventDefault(); if (!dragCol || dragCol === k) { setDragCol(null); return } setCols(prev => { const arr = [...prev]; const from = arr.indexOf(dragCol); const to = arr.indexOf(k); arr.splice(from, 1); arr.splice(to, 0, dragCol); return arr }); setDragCol(null) }, onClick: () => toggleSort(k), style: { padding: '9px 12px', cursor: 'pointer', color: 'var(--q-text-secondary)', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font-interface)', whiteSpace: 'nowrap', userSelect: 'none', backgroundColor: dragCol === k ? 'var(--q-hover)' : 'var(--q-bg)' } }, c ? c.label : '') }))
    const tbody = rows.map((i, ri) => React.createElement('tr', { key: i.id, onMouseEnter: () => setHoverRow(i.id), onMouseLeave: () => setHoverRow(null), style: { transition: 'none' } }, cols.map((k, idx) => { const isHov = hoverRow === i.id; const isLast = ri === rows.length - 1; if (k === 'title') { return React.createElement('td', { key: k, style: cellStyle('left', isLast, isHov) }, React.createElement('span', { key: 't', style: { color: 'var(--q-text)', wordBreak: 'break-word' } }, i.title)) } if (k === 'actions') { return React.createElement('td', { key: k, style: cellStyle('left', isLast, isHov) }, actions(i)) } return React.createElement('td', { key: k, style: cellStyle('left', isLast, isHov) }, cellVal(i, k)) })))
    return React.createElement('div', { key: 'tbl', style: { width: '100%', overflowX: 'auto' } }, React.createElement('table', { style: { width: '100%', borderCollapse: 'separate', borderSpacing: 1, backgroundColor: 'var(--q-border)', tableLayout: 'fixed' } }, [React.createElement('thead', { key: 'th' }, thead), React.createElement('tbody', { key: 'tb' }, tbody.length ? tbody : React.createElement('tr', { key: 'e' }, React.createElement('td', { colSpan: cols.length, style: { padding: 16, textAlign: 'center', color: 'var(--q-text-tertiary)', fontSize: 13, fontFamily: 'var(--font-interface)', backgroundColor: 'var(--q-bg)' } }, 'No tasks in this group.')))]))
  }
