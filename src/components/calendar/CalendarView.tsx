...
function MtText({ label, onClick }: { label: string; onClick: (e: any) => void }) {
  const [h, setH] = useState(false)
  return React.createElement('span', { onClick, onMouseEnter: () => setH(true), onMouseLeave: () => setH(false), style: { color: h ? 'var(--q-text)' : 'var(--q-text-secondary)', cursor: 'pointer', fontSize: 12.5, fontFamily: 'var(--font-interface)', wordBreak: 'break-word', transition: 'none' } }, label)
}
...
export function CalendarView(props: { activePanel: string; onSelectPanel: (p: string) => void; agents?: any[]; onOpenSession?: (key: string) => void }) {
  ...
  const cellVal = (i: Item, key: string): React.ReactNode => {
    if (key === 'when') {
      ...
      const content = React.createElement('div', { key: 'dt', style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2 } }, [
        React.createElement('span', { key: 'tt', style: { wordBreak: 'break-word', fontSize: 12.5, fontFamily: 'var(--font-interface)', color: 'var(--q-text-secondary)' } }, timeStr),
        React.createElement('span', { key: 'dd', style: { wordBreak: 'break-word', fontSize: 12.5, fontFamily: 'var(--font-interface)', color: 'var(--q-text-secondary)', opacity: 0.8 } }, dateStr),
      ])
      if (i.kind === 'sched') {
  ...
  const cellStyle = (align = 'left', isLast = false, isLastCol = false): React.CSSProperties => ({ padding: '9px 12px', color: 'var(--q-text-secondary)', fontSize: 13.5, fontFamily: 'var(--font-interface)', textAlign: align as any, whiteSpace: 'normal', wordBreak: 'break-word', borderBottom: isLast ? 'none' : '1px solid var(--q-border-solid)', borderRight: isLastCol ? 'none' : '1px solid var(--q-border-solid)' })
