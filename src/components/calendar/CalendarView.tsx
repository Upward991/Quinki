    ...
    open ? React.createElement(React.Fragment, { key: 'm' }, [
      ...
      React.createElement('div', { key: 'd', style: { position: 'absolute', zIndex: 151, minWidth: 230, maxHeight: 320, backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-modal)', border: '1px solid var(--q-border)', padding: 8, display: 'flex', flexDirection: 'column', gap: 3, ...pos } }, [
        React.createElement('div', { key: 'top', style: { display: 'flex', alignItems: 'center', gap: 5, marginBottom: 5 } }, [
          ...
          React.createElement('button', { key: 'cl', title: 'Clear ' + label, onClick: () => { onChange([]); setOpen(false) }, style: { height: 34, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '0 12px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-border)', background: 'var(--q-bg-elevated)', color: 'var(--q-accent-danger)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-interface)', whiteSpace: 'nowrap' } }, [React.createElement(X, { key: 'x', size: 14 }), React.createElement('span', { key: 't' }, 'Clear')]),
        ]),
