...
export function CalendarView(props: { activePanel: string; onSelectPanel: (p: string) => void; agents?: any[]; onOpenSession?: (key: string) => void }) {
  ...
  const tableFor = (rows: Item[]) => {
    ...
    const thead = React.createElement('tr', { key: 'thr' }, renderCols.map((k, idx) => { const c = COLS.find(x => x.key === k); const isLastCol = idx === renderCols.length - 1; if (k === 'actions') return React.createElement('th', { key: k, style: { padding: '9px 12px', color: 'var(--q-text-secondary)', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font-interface)', whiteSpace: 'nowrap', userSelect: 'none', minWidth: COL_MIN.actions, borderBottom: '1px solid var(--q-border-solid)' } }, 'Actions'); return React.createElement('th', { key: k, onClick: () => toggleSort(k), style: { padding: '9px 12px', cursor: 'grab', color: 'var(--q-text-secondary)', fontSize: 13.5, fontWeight: 700, fontFamily: 'var(--font-interface)', whiteSpace: 'nowrap', userSelect: 'none', minWidth: COL_MIN[k] || 80, borderBottom: '1px solid var(--q-border-solid)', borderRight: isLastCol ? 'none' : '1px solid var(--q-border-solid)' } }, c ? (sortKey === k ? React.createElement('span', { key: 'lb' }, [c.label, React.createElement('span', { key: 'so', style: { fontSize: 10, color: 'var(--q-text-tertiary)', marginLeft: 4 } }, sortDir === 1 ? '▲' : '▼')]) : c.label) : '') }))
