...
// Bubble normale a tutta larghezza, sfondo chat, titolo "Long Horizon Mode" in alto a sinistra.
  ...
  return (
    <div style={{ maxWidth: 'var(--spacing-chat-max)', minWidth: 0, margin: '8px 0' }}>
      <div style={{ backgroundColor: 'var(--q-bg)', border: '1px solid var(--q-border)', borderRadius: 'var(--radius-lg)', padding: '12px 16px' }}>
        ...
        <div style={{ color: 'var(--q-text)', fontSize: 13, lineHeight: 1.6, fontFamily: 'var(--font-interface)', whiteSpace: 'pre-wrap' }}>{text}</div>
      </div>
    </div>
  )
}
...
function UserMessage({ message, onCopy, searchQuery, activeOcc, isDateMatch }: { message: Message; onCopy?: (t: string) => void; searchQuery?: string; activeOcc?: number; isDateMatch?: boolean }) {
  ...
  return (
    <div className="user-message-content" style={{ width: '100%', padding: '10px 16px', backgroundColor: 'var(--q-bubble-user)', borderRadius: '12px', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.02)', border: isDateMatch ? '2px solid var(--q-search-highlight-bg)' : '1px solid var(--q-border)', animation: 'msgSent 300ms cubic-bezier(0.16, 1, 0.3, 1)' }}>
...
function renderBlocks(blocks: any[], opts: { isError?: boolean; isStreaming?: boolean; timestamp: string; agentName?: string; agentModel?: string; thinkingLevel?: string; thinkingTranslated?: string; onCopy?: (t: string) => void; searchQuery?: string; activeOcc?: number }) {
  const { isError, isStreaming, timestamp, agentName, agentModel, thinkingLevel, thinkingTranslated, onCopy, searchQuery, activeOcc } = opts
        ...
        <div key={`b-${i}`}>
          ...
          {showFooter && (
            <Footer
              ...
              thinkingTranslated={thinkingTranslated}
              onCopy={onCopy}
            />
          )}
        </div>
  ...
  return (
    <div className="assistant-content" style={{ maxWidth: 'var(--spacing-chat-max)', minWidth: 0, animation: 'materialize 400ms cubic-bezier(0.16, 1, 0.3, 1)', userSelect: 'text', WebkitUserSelect: 'text', border: isDateMatch ? '2px solid var(--q-search-highlight-bg)' : 'none', borderRadius: isDateMatch ? 'var(--radius-md)' : '0', padding: isDateMatch ? '8px' : '0' }}>
      ...
      {(message as any).blocks?.length > 0
        ? renderBlocks((message as any).blocks, { isError, isStreaming: !!message.isStreaming, timestamp: message.timestamp, agentName: message.agentName, agentModel: message.agentModel, thinkingLevel: message.thinkingLevel, thinkingTranslated: (message as any).thinkingTranslated, onCopy, searchQuery, activeOcc })
        : <>
      ...
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={() => setCollapsed(!collapsed)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); try { navigator.clipboard.writeText(content) } catch {} }}
            ...
            style={{ opacity: (hovered || (document.documentElement.classList.contains(`q-phone`) && !collapsed)) ? 1 : 0, background: 'none', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: 'var(--radius-sm)', color: copyHovered ? color : `rgba(${baseColorRgb}, 0.60)` }}>
            <Copy size={14} />
          </button>
        </div>
      </div>
    ...
    <div style={{ marginTop: '12px', padding: '4px' }}>
      ...
      {!collapsed && (
        <div className="toggle-content" style={{ marginTop: '4px', padding: '8px 8px 8px 16px', borderLeft: `2px solid ${baseColor}`, userSelect: 'text', WebkitUserSelect: 'text', animation: 'materialize 300ms cubic-bezier(0.16, 1, 0.3, 1)' }}>
          {/* User message (task) */}
          <div style={{ width: '100%', marginBottom: '8px', padding: '10px 16px', backgroundColor: 'var(--q-bubble-user)', borderRadius: '12px', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.02)', border: '1px solid var(--q-border)' }}>
          ...
          {delegation.blocks?.length > 0
            ? renderBlocks(delegation.blocks, { isStreaming: !!streaming, timestamp, agentName: delegation.agentName, agentModel: delegation.agentModel, thinkingLevel: delegation.thinkingLevel, thinkingTranslated: (delegation as any).thinkingTranslated, onCopy, searchQuery, activeOcc })
            : delegation.response && (
              <div style={{ padding: '4px 0' }}>
          ...
          {(!delegation.blocks || delegation.blocks.length === 0) && delegation.response && (
            <Footer
              ...
              thinkingTranslated={(delegation as any).thinkingTranslated}
            />
          )}
        </div>
      )}
...
function Footer({ content, timestamp, agentName, agentModel, thinkingLevel, thinkingTranslated, sentEffort, reasoningUsed, reasoningTokens, onCopy }: {
  ...
  const displayThinking = (() => {
    ...
    // Se la risposta contiene reasoning → il livello attivo (Max). Se NON lo contiene
    // → il modello non ha usato thinking in questa risposta (None).
    if (reasoningUsed === true && sentEffort) return sentEffort.charAt(0).toUpperCase() + sentEffort.slice(1)
    if (reasoningUsed === false) return 'None'
    if (sentEffort) return sentEffort.charAt(0).toUpperCase() + sentEffort.slice(1)
    if (!thinkingLevel || thinkingLevel === 'off') return 'Off'
    if (thinkingTranslated) {
      const v = String(thinkingTranslated)
      if (['true', 'enabled', 'on', '1'].includes(v.toLowerCase())) return 'On'
  ...
  if (thinkingLevel || sentEffort || reasoningUsed !== undefined) infoParts.push(displayThinking)
