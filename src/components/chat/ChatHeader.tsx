    ...
    <>
      <div className="flex items-center" style={{ minWidth: 0, width: '100%' }}>
        ...
        {props.showRollback && (
          <>
            <div style={panelStyle}>
              <IconBtn icon={RotateCcw} onClick={() => setChoiceOpen(true)} title="App Expert: sync or rollback" />
            </div>
            <div style={{ width: '8px', flexShrink: 0 }} />
          </>
        )}
        ...
        <div style={{ ...panelStyle, flex: 1, minWidth: 0 }}>
          ...
          {!mob && !props.welcomeMode && (
            <div style={{ position: 'relative', flexShrink: 0 }}>
              ...
              >
                {props.notifyMode === 'none' ? <BellOff size={16} /> : <Bell size={16} />}
              </button>
              {notifMenuOpen && notifMenuPos && createPortal(
                <>
                  <div style={{ position: 'fixed', inset: 0, zIndex: 9998 }} onClick={() => setNotifMenuOpen(false)} />
                  <div style={{ position: 'fixed', left: notifMenuPos.left, top: notifMenuPos.top, zIndex: 9999, backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-modal)', border: '1px solid var(--q-border)', padding: '4px 0', minWidth: '180px' }}>
                    <button onClick={() => { props.onSetNotifyMode && props.onSetNotifyMode('none'); setNotifMenuOpen(false) }} style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', padding: '8px 12px', border: 'none', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-text)', fontSize: '13px', fontFamily: 'var(--font-interface)', textAlign: 'left' }}><BellOff size={14} /> Muted</button>
                    <button onClick={() => { props.onSetNotifyMode && props.onSetNotifyMode('all'); setNotifMenuOpen(false) }} style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', padding: '8px 12px', border: 'none', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-text)', fontSize: '13px', fontFamily: 'var(--font-interface)', textAlign: 'left' }}><Bell size={14} /> All notifications</button>
                  </div>
                </>,
                document.body
              )}
        ...
        {/* Sync or Rollback choice modal (copy of the Settings App Expert section, without section title/description) */}
      {choiceOpen && (
        <>
          <div style={{ position: 'fixed', inset: 0, zIndex: 400, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setChoiceOpen(false)} />
          <div style={{ position: 'fixed', zIndex: 401, inset: 0, margin: 'auto', height: 'fit-content', backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-modal)', border: '1px solid var(--q-border)', padding: '24px', maxWidth: '440px', width: '90%' }}>
            <div style={{ color: 'var(--q-text)', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Sync</div>
            <div style={{ color: 'var(--q-text-tertiary)', fontSize: '12px', fontFamily: 'var(--font-interface)', marginTop: '4px' }}>
              Copies the latest Quinki build (binary + sidecar) into the App Expert. Use this after updating Quinki to keep the Expert in sync. The Expert will restart to apply the changes.
            </div>
            <div style={{ height: '12px' }} />
            <button onClick={() => { setChoiceOpen(false); setSyncConfirm(true) }}
              ...
              style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Sync Expert App</button>
            <div style={{ height: '16px' }} />
            <div style={{ color: 'var(--q-text)', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Rollback</div>
            <div style={{ color: 'var(--q-text-tertiary)', fontSize: '12px', fontFamily: 'var(--font-interface)', marginTop: '4px' }}>
              Restores the App Expert to the previous version (before the last sync). Use this if a sync broke the app or the chat stopped working. The app will restart with the previous code. Your data is never affected.
            </div>
            <div style={{ height: '12px' }} />
            <button onClick={() => { setChoiceOpen(false); setRollbackConfirm(true) }}
              onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--q-tab-accent)'; e.currentTarget.style.color = 'var(--q-bg)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--q-tab-accent)' }}
              style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Rollback App Expert</button>
            <div style={{ height: '12px' }} />
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button onClick={() => setChoiceOpen(false)} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.06)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent' }}
      ...
      {/* Sync App Expert confirm modal (same flow as Rollback) */}
      {syncConfirm && (
        <>
          <div style={{ position: 'fixed', inset: 0, zIndex: 400, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setSyncConfirm(false)} />
          <div style={{ position: 'fixed', zIndex: 401, inset: 0, margin: 'auto', height: 'fit-content', backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-modal)', border: '1px solid var(--q-border)', padding: '24px', maxWidth: '440px', width: '90%' }}>
            <div style={{ color: 'var(--q-text)', fontSize: '16px', fontWeight: 600, fontFamily: 'var(--font-interface)', marginBottom: '12px' }}>Sync App Expert?</div>
            <div style={{ color: 'var(--q-text-secondary)', fontSize: '13px', fontFamily: 'var(--font-interface)', lineHeight: 1.6, marginBottom: '16px' }}>
              This copies the latest Quinki build (binary + sidecar) into the App Expert. The Expert will restart with the new code. Your data is not affected.
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              <button onClick={() => setSyncConfirm(false)} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.06)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent' }}
                style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-border)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-accent-danger)', fontSize: '13px', fontFamily: 'var(--font-interface)' }}>Cancel</button>
              <button onClick={() => { setSyncConfirm(false); setSyncState(2); setExpertRunning(true); try { let t0 = Date.now(); invoke('sync_expert_app').then(async (r: any) => { let elapsed = Date.now() - t0; if (elapsed < 800) await new Promise(res => setTimeout(res, 800 - elapsed)); setSyncMsg(String(r)); setSyncState(3) }).catch((e: any) => { setSyncMsg('Sync failed: ' + e); setSyncState(3) }) } catch {} }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--q-tab-accent)'; e.currentTarget.style.color = 'var(--q-bg)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--q-tab-accent)' }}
                style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Sync</button>
            </div>
          </div>
        </>
      )}

      {/* Sync progress / complete modal (same flow as Rollback) */}
      {syncState >= 2 && (
        <>
          <div style={{ position: 'fixed', inset: 0, zIndex: 400, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => { if (syncState !== 2) setSyncState(0) }} />
          <div style={{ position: 'fixed', zIndex: 401, inset: 0, margin: 'auto', height: 'fit-content', backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-modal)', border: '1px solid var(--q-border)', padding: '24px', maxWidth: '440px', width: '90%' }}>
            <div style={{ color: 'var(--q-text)', fontSize: '16px', fontWeight: 600, fontFamily: 'var(--font-interface)', marginBottom: '12px' }}>{syncState === 2 ? 'Syncing...' : (syncMsg.includes('failed') ? 'Sync Failed' : 'Sync Complete')}</div>
            <div style={{ color: 'var(--q-text-secondary)', fontSize: '13px', fontFamily: 'var(--font-interface)', lineHeight: 1.6, marginBottom: '16px' }}>
              {syncState === 2 ? 'Please wait while the Expert app is being synced...' : (syncMsg || 'Expert app synced successfully.') + (expertRunning ? ' Click Restart to apply the changes.' : ' The Expert app will use the new version next time you open it.')}
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              {syncState === 2 && <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--q-text-secondary)', fontSize: '13px', fontFamily: 'var(--font-interface)' }}><div style={{ width: '14px', height: '14px', border: '2px solid var(--q-text-tertiary)', borderTopColor: 'var(--q-tab-accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />Syncing...</div>}
              {syncState === 3 && !expertRunning && <button onClick={() => setSyncState(0)} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--q-tab-accent)'; e.currentTarget.style.color = 'var(--q-bg)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--q-tab-accent)' }} style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Done</button>}
              {syncState === 3 && expertRunning && <button onClick={async () => { try { await invoke('restart_expert_app') } catch {} try { if (props.isExpertApp) { const nat = (window as any).QuinkiNative; if (nat && nat.closeApp) { nat.closeApp(); return } } } catch {} setSyncState(0) }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--q-tab-accent)'; e.currentTarget.style.color = 'var(--q-bg)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--q-tab-accent)' }} style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Restart</button>}
            </div>
          </div>
        </>
      )}
      ...
      {rollbackState >= 2 && (
        <>
          ...
          <div style={{ position: 'fixed', zIndex: 401, inset: 0, margin: 'auto', height: 'fit-content', backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-modal)', border: '1px solid var(--q-border)', padding: '24px', maxWidth: '400px', width: '90%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              ...
              {rollbackState === 3 && expertRunning && <button onClick={async () => { try { await invoke('restart_expert_app') } catch {} try { if (props.isExpertApp) { const nat = (window as any).QuinkiNative; if (nat && nat.closeApp) { nat.closeApp(); return } } } catch {} setRollbackState(0) }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--q-tab-accent)'; e.currentTarget.style.color = 'var(--q-bg)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--q-tab-accent)' }} style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Restart</button>}
            </div>
          </div>
        </>
      )}
...
{agentPanelBody}
      ...
      {exportOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, backgroundColor: 'var(--q-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={(e: any) => { if (e.target === e.currentTarget) setExportOpen(false) }}>
          <div style={{ backgroundColor: 'var(--q-bg-elevated)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-modal)', overflow: 'hidden', maxWidth: '380px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '0 16px 12px 16px' }}>
              <ExportBtn label="Cancel" color="var(--q-accent-danger)" hoverRgb="217,107,107" onClick={(e: any) => { if (e.target === e.currentTarget) setExportOpen(false) }} />
              <div style={{ width: '8px', flexShrink: 0 }} />
        ...
        view={menuView === 'context' ? (
          ...
          <div style={{ padding: '0 8px 8px 8px' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <span style={{ color: matchCount > 0 ? 'var(--q-text-secondary)' : 'var(--q-text-tertiary)', fontSize: '12px', fontFamily: 'var(--font-code)', whiteSpace: 'nowrap', opacity: matchCount > 0 ? 1 : 0.3, flexShrink: 0 }}>{matchCount > 0 ? (currentMatch + 1) + '/' + matchCount : '0/0'}</span>
              <div style={{ width: '8px', flexShrink: 0 }} />
              <button onClick={() => { setMenuOpen(false); setMenuView(null); props.onMatchNavigate?.('prev') }} style={{ background: 'none', border: 'none', cursor: matchCount > 0 ? 'pointer' : 'default', padding: '0', color: matchCount > 0 ? 'var(--q-text-secondary)' : 'var(--q-text-tertiary)', opacity: matchCount > 0 ? 1 : 0.3, lineHeight: '0', flexShrink: 0, display: 'flex' }}><ChevronUp size={16} /></button>
              <button onClick={() => { setMenuOpen(false); setMenuView(null); props.onMatchNavigate?.('next') }} style={{ background: 'none', border: 'none', cursor: matchCount > 0 ? 'pointer' : 'default', padding: '0', color: matchCount > 0 ? 'var(--q-text-secondary)' : 'var(--q-text-tertiary)', opacity: matchCount > 0 ? 1 : 0.3, lineHeight: '0', flexShrink: 0, display: 'flex' }}><ChevronDown size={16} /></button>
            </div>
            <div style={{ height: '8px' }} />
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <Calendar size={16} style={{ color: 'var(--q-text-tertiary)', flexShrink: 0 }} />
...
function AgentPickerModal({ agents, initialSelected, onClose, onAdd, embedded }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, backgroundColor: 'var(--q-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={(e: any) => { if (e.target === e.currentTarget) onClose() }}>
...
export function ModelPickerModal({ currentModel, models, onClose, onConfirm, embedded }: {
  ...
  return (
    <div ref={overlayRef} style={{ position: 'fixed', inset: 0, zIndex: 200, backgroundColor: 'var(--q-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={(e: any) => { if (e.target === e.currentTarget) onClose() }}>
