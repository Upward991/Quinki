...
import { useRef, useEffect, useState, useLayoutEffect, useCallback, memo } from 'react'
...
// NOOP stabile: usarlo per onCopy → non rompe React.memo su MessageBubble
const NOOP_COPY = () => {}
      ...
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={() => setCollapsed(!collapsed)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); try { let respText = ''; for (const m of run.messages || []) { if (m.role === 'assistant' && m.content) respText += (m.content || '') + '\n\n' } const text = respText.trim() || run.label; window.dispatchEvent(new CustomEvent('quinki-task-clip', { detail: { id: run.id, label: run.label, text } })) } catch {} }}
            ...
            title="Clip to chat"
            style={{ opacity: (hovered || (document.documentElement.classList.contains(`q-phone`) && !collapsed)) ? 1 : 0, background: 'none', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: 'var(--radius-sm)', color: clipHovered ? color : 'var(--q-text-tertiary)' }}>
            <Paperclip size={14} />
          </button>
          )}
          <button onClick={(e) => { e.stopPropagation(); try { let fullText = ''; for (const m of run.messages || []) { fullText += (m.role === 'user' ? 'User: ' : 'Agent: ') + (m.content || '') + '\n' } navigator.clipboard.writeText(fullText) } catch {} }}
            ...
            style={{ opacity: (hovered || (document.documentElement.classList.contains(`q-phone`) && !collapsed)) ? 1 : 0, background: 'none', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: 'var(--radius-sm)', color: copyHovered ? color : 'var(--q-text-tertiary)' }}>
            <Copy size={14} />
          </button>
        </div>
      </div>
        ...
        <div style={{ marginTop: '4px', padding: '8px 8px 8px 16px', borderLeft: `2px solid ${color}`, userSelect: 'text', WebkitUserSelect: 'text' }}>
          ...
          {run.messages.map((msg: any, mIdx: number) => (
            <div key={msg.id + '-' + mIdx} style={{ marginBottom: '12px' }}>
              <MessageBubble message={msg} onCopy={NOOP_COPY} searchQuery={''} msgIndex={mIdx} activeMatchMsgIdx={-1} activeMatchOccurrence={-1} isDateMatch={false} />
            </div>
          ))}
        </div>
      )}
...
interface ChatAreaProps {
  ...
  onCompact?: () => void
  onLoadOlder?: () => void
  onJumpToMessage?: (ts: number) => void
...
// Righe memoizzate: a ogni re-render del ChatArea i messaggi INVARIATI non si
// ricalcolano (wrapper + marker unread/bookmark + bubble) — il lag non scala col contesto.
const ChatRow = memo(function ChatRow({ item, index, isBookmark, isFirstUnread, searchQuery, activeMatchMsgIdx, activeMatchOccurrence, isDateMatch, sessionKey }: {
  item: any, index: number, isBookmark: boolean, isFirstUnread: boolean, searchQuery: string,
  activeMatchMsgIdx: number, activeMatchOccurrence: number, isDateMatch: boolean, sessionKey: string
}) {
  return (
    <div key={item.kind === 'msg' ? item.msg!.id : item.kind === 'sys' ? 'lh-sys' : 'chat-' + item.run!.id} data-msg-idx={item.mIdx ?? -1} data-item-idx={index} className="q-cv-item" style={{ marginBottom: '12px' }}>
      {isBookmark && (
      ...
      {item.kind === 'msg' ? (
        <MessageBubble message={item.msg} onCopy={NOOP_COPY} searchQuery={searchQuery} msgIndex={item.mIdx ?? 0} activeMatchMsgIdx={activeMatchMsgIdx} activeMatchOccurrence={activeMatchOccurrence} isDateMatch={isDateMatch} />
      ) : item.kind === 'sys' ? (
        ...
        <TaskResultToggle run={item.run} sessionKey={sessionKey} defaultOpen />
      )}
    </div>
  )
})
...
export function ChatArea(props: ChatAreaProps) {
  ...
  // A4.3: se è la tab Expert nella MAIN e l'app Expert è aperta (sidecar 9183),
  // NON caricare la sessione __app_expert__ (107MB+) — la sessione vive lì.
  const [expertAlive, setExpertAlive] = useState<boolean | null>(null)
  useEffect(() => {
    if (!(sessionIdKey === '__app_expert__' && !props.isExpertApp)) { setExpertAlive(null); return }
    let cancelled = false
    sidecarCall('isExpertAlive').then((r: any) => { if (!cancelled) setExpertAlive(!!r?.alive) }).catch(() => { if (!cancelled) setExpertAlive(false) })
    // Handoff LIVE (29 ago): se l'app Expert si accende/spegne mentre questa tab è
    // aperta nella main, la main lo scopre entro 3s (prima controllava solo
    // all'apertura → lo streaming restava nella main con l'Expert acceso).
    const poll = setInterval(() => {
      if (cancelled) return
      sidecarCall('isExpertAlive').then((r: any) => { if (!cancelled) setExpertAlive(!!r?.alive) }).catch(() => {})
    }, 3000)
    return () => { cancelled = true; clearInterval(poll) }
  }, [sessionIdKey, props.isExpertApp, sidecarCall])
    ...
    <div style={{ position: 'absolute', inset: '2px 2px 0 2px', zIndex: 20, backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-modal)', display: 'flex', flexDirection: 'column', animation: 'taskPanelExpand 180ms ease-out', transformOrigin: 'bottom', overflow: 'hidden' }}>
      <div ref={scrollRef} className="q-scroll" style={{ flex: 1, overflowY: 'auto', padding: '16px 16px', scrollbarGutter: 'stable' }}>
        {taskRuns.length === 0 ? (
          ...
          <TaskResultToggle key={run.id} run={run} sessionKey={props.session?.id} showClip />
        ))}
      </div>
  ...
  const activeMatchInfo = (() => {
    if (matches.length === 0 || activeMatchIdx < 0 || activeMatchIdx >= matches.length) return null
  ...
  // B0.9: scroll anchoring — dopo il prepend dei messaggi più vecchi, mantieni la posizione
  const prevMsgCountRef = useRef(props.messages.length)
  useEffect(() => {
    const el = scrollRef.current
    if (!el) return
    const grew = props.messages.length > prevMsgCountRef.current
    prevMsgCountRef.current = props.messages.length
    if (grew && anchorRef.current) {
  ...
  const isLoadingChat = !!chatLoading && props.messages.length === 0 && !props.welcomeMode
  ...
  // A4.3: tab Expert nella MAIN con l'app Expert aperta → non caricare la sessione
  if (sessionIdKey === '__app_expert__' && !props.isExpertApp && expertAlive === true) {
    return (
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ maxWidth: '420px', textAlign: 'center', padding: '24px' }}>
            ...
            <button onClick={() => { try { props.onSelectPanel('home') } catch {} }}
              ...
              style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)', cursor: 'pointer' }}>OK</button>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="h-full flex flex-col" style={{ maxWidth: 'var(--spacing-chat-max)', margin: '0 auto', width: '100%', position: 'relative' }} onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
      onKeyDown={(e) => {
        // Tab ovunque nella chat → switch plan/build (non solo nel composer).
        // Salta se il focus è in un altro input (cerca, modali, settings).
        if (e.key === 'Tab') {
          const t = e.target as HTMLElement;
          const inOtherInput = t && (t.tagName === 'INPUT' || t.tagName === 'SELECT') && !(t as HTMLTextAreaElement).className?.toString().includes('q-composer');
          if (!inOtherInput) { e.preventDefault(); props.onModeChange(props.mode === 'plan' ? 'build' : 'plan'); }
        }
      }}>
      {/* Drag-drop overlay */}
      {dragOver && (
...
{/* Welcome: composer centered */}
      {isLoadingChat ? (
        <div style={{ height: '100%', padding: '20px 16px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-evenly', height: '100%', maxWidth: 'var(--spacing-chat-max)', margin: '0 auto' }}>
            <div style={{ alignSelf: 'flex-end', width: '42%', height: '30px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.07)', animation: 'breathe 1.4s ease-in-out infinite 0.0s' }} />
            <div style={{ alignSelf: 'flex-start', width: '68%', height: '38px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.08s' }} />
            <div style={{ alignSelf: 'flex-start', width: '52%', height: '46px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.16s' }} />
            <div style={{ alignSelf: 'flex-end', width: '35%', height: '30px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.07)', animation: 'breathe 1.4s ease-in-out infinite 0.24s' }} />
            <div style={{ alignSelf: 'flex-start', width: '78%', height: '38px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.32s' }} />
            <div style={{ alignSelf: 'flex-start', width: '45%', height: '46px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.4s' }} />
            <div style={{ alignSelf: 'flex-end', width: '55%', height: '30px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.07)', animation: 'breathe 1.4s ease-in-out infinite 0.48s' }} />
            <div style={{ alignSelf: 'flex-start', width: '70%', height: '38px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.56s' }} />
            <div style={{ alignSelf: 'flex-start', width: '38%', height: '46px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.64s' }} />
            <div style={{ alignSelf: 'flex-end', width: '47%', height: '30px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.07)', animation: 'breathe 1.4s ease-in-out infinite 0.72s' }} />
            <div style={{ alignSelf: 'flex-start', width: '65%', height: '38px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.8s' }} />
            <div style={{ alignSelf: 'flex-start', width: '58%', height: '46px', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.05)', animation: 'breathe 1.4s ease-in-out infinite 0.88s' }} />
          </div>
        </div>
      ) : isEmpty ? (
        taskPanelOpen ? (
          ...
          <div style={{ flex: 1, minHeight: 0, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', ...(taskPanelOpen ? { margin: '2px', backgroundColor: 'var(--q-bg-panel)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-modal)', animation: 'taskPanelExpand 180ms ease-out', transformOrigin: 'bottom' } : {}) }}>
            ...
            <div style={{ flex: 1, minHeight: 0, position: 'relative', display: 'flex', flexDirection: 'column' }}>
              ...
              <div ref={scrollRef} className="q-scroll" style={{ flex: 1, overflowY: 'auto', padding: '4px 16px ' + (hasTasks ? 8 : 0) + 'px 16px', scrollbarGutter: 'stable', display: taskPanelOpen ? 'none' : 'block' }}
                onScroll={e => { const el = e.currentTarget; const overflow = el.scrollHeight - el.clientHeight; setShowScrollBtn(overflow > 260 && el.scrollTop < overflow - 100); pinnedRef.current = el.scrollTop + el.clientHeight >= el.scrollHeight - 120; if (el.scrollTop < 40 && props.onLoadOlder && !loadingOlderRef.current && props.messages.length > 0) { loadingOlderRef.current = true; anchorRef.current = { height: el.scrollHeight, top: el.scrollTop }; setTimeout(() => { loadingOlderRef.current = false }, 600); props.onLoadOlder() } }}>
                {(() => {
                  ...
                  if (props.longHorizonSystemMessage && props.longHorizonStartedAt) {
                    chatItems.push({ kind: 'sys', ts: props.longHorizonStartedAt || Date.now(), sysMsg: props.longHorizonSystemMessage })
                  }
                  ...
                  return (
                    ...
                    {chatItems.map((item, i) => {
                      ...
                      return (
                        <ChatRow
                          key={item.kind === 'msg' ? item.msg!.id : item.kind === 'sys' ? 'lh-sys' : 'chat-' + item.run!.id}
                          item={item} index={i} isBookmark={isBookmark} isFirstUnread={isFirstUnread}
                          searchQuery={searchQuery}
                          activeMatchMsgIdx={activeMatchInfo?.msgIdx ?? -1} activeMatchOccurrence={activeMatchInfo?.occurrence ?? -1}
                          isDateMatch={!searchQuery.trim() && hasDateFilter && dateMatchIndices.includes(item.mIdx ?? -1) && (item.mIdx ?? -1) === dateMatchIndices[Math.min(dateMatchIdx, dateMatchIndices.length - 1)]}
