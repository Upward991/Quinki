...
export function LogPanel(props: LogPanelProps) {
          ...
          call('getDebugLogSince', { ts: lastTs }).then((res: any) => {
            ...
            if (res.entries && res.entries.length) {
              setEntries(prev => {
                ...
                return add.length ? [...prev, ...add].slice(-100) : prev
              })
            }
          }).catch(() => {})
        }, 1500)
  ...
  // B0.9: search sidecar — cerca in TUTTO il file log (non solo i 100 caricati)
  useEffect(() => {
    const q = search.trim()
    if (!q) { setHistoryMatches([]); return }
    const t = setTimeout(() => {
      call('searchDebugLog', { query: q, limit: 30 }).then((r: any) => {
        setHistoryMatches(r?.matches || [])
      }).catch(() => {})
    }, 300)
  ...
  const loadOlderLogs = useCallback(() => {
    if (!call || loadingOlderRef.current) return
    const oldest = entries[0]
    if (!oldest || typeof oldest.ts !== 'number') return
    loadingOlderRef.current = true
    setTimeout(() => { loadingOlderRef.current = false }, 600)
    call('getDebugLogBefore', { from: oldest.ts, limit: 100 }).then((r: any) => {
      if (r?.entries && r.entries.length) {
        setEntries(prev => {
          const seen = new Set(prev.map((e: any) => e.ts + ':' + e.tag))
          const add = r.entries.filter((e: any) => !seen.has(e.ts + ':' + e.tag))
  ...
  const onScroll = () => {
    ...
    // B0.8: paginazione — in cima carica i log più vecchi
    if (el.scrollTop < 40 && entries.length > 0) loadOlderLogs()
  }
  useEffect(() => {
  ...
  function deriveLevel(tag: string): string {
    ...
    if (tag === 'agent_llm_config' || tag === 'msg-content' || tag === 'thinking-map-xhigh-fix' || tag === 'thinking-map-resolved' || tag === 'thinking-trace' || tag === 'thinking-caps-applied' || tag === 'response-thinking-verified' || tag === 'thinking-level-intent' || tag === 'thinking-http-intent') return 'llm'
  ...
  function formatPayload(data: any): string {
    ...
    // Special formatting for thinking-trace: chain readable at a glance
    if (data && data.sentEffort !== undefined && data.requested !== undefined) {
      const used = data.reasoningActuallyUsed === true
        ? `USED ✓ (${data.reasoningTokens != null ? data.reasoningTokens + ' tok, ' : ''}${data.reasoningLength} chars)`
        : 'NOT USED in this response'
      return `[${String(data.model || '')}] requested: ${data.requested || '-'} | sent: ${data.sentEffort || '-'} | caps max: ${data.capsMax || '-'} | reasoning: ${used} | verdict: ${data.verdict || '-'}`
    }
    ...
    if (data && data.source !== undefined && data.model !== undefined && data.agentId !== undefined && data.systemPromptLen !== undefined) {
      ...
      const effortLabel = (() => {
        const t = String(data.thinkingTranslated || '')
        if (!t) return '(none)'
        if (['true', 'enabled', 'on', '1'].includes(t.toLowerCase())) return 'On'
      ...
      info += '\n  Agent override: ' + (data.agentOverrideModel || 'Chat default')
      info += '\n'
      info += '\n── Thinking ──'
      info += '\n  Used: ' + (data.thinkingLevel || '(default)')
      info += '\n  Effort sent to model: ' + effortLabel
      const overrideThinkingLabel = data.agentOverrideThinking === 'on' ? 'On' : data.agentOverrideThinking === 'off' ? 'Off' : 'Chat default'
        ...
        <div style={{ ...panelStyle, flex: 1 }}>
          ...
          <HeaderBtn label="refresh" icon={<RefreshCw size={14} />} onClick={() => {
            ...
            call('getFullDebugLog', {}).then((r: any) => {
              if (r && r.log) setEntries(r.log.slice(-100))
            }).catch(() => {})
          }} />
          <div style={{ width: '4px', flexShrink: 0 }} />
          <HeaderBtn label="live" icon={<Pulse size={14} />} active={liveMode} onClick={() => {
            ...
            if (nv && call) call('getFullDebugLog', {}).then((r: any) => { if (r && r.log) setEntries(r.log.slice(-100)) }).catch(() => {})
          }} />
          <div style={{ width: '4px', flexShrink: 0 }} />
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, backgroundColor: 'var(--q-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowExport(false)}>
          <div style={{ backgroundColor: 'var(--q-bg-elevated)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-modal)', animation: 'modalEnter 250ms cubic-bezier(0.16, 1, 0.3, 1)', padding: '24px', maxWidth: '400px', width: '90%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
              <button onClick={() => setShowExport(false)} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.06)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent' }} style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-border)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-accent-danger)', fontSize: '13px', fontFamily: 'var(--font-interface)' }}>Cancel</button>
              <div style={{ width: '8px' }} />
              ...
              }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--q-tab-accent)'; e.currentTarget.style.color = 'var(--q-bg)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = 'var(--q-tab-accent)' }} style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-tab-accent)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-tab-accent)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-interface)' }}>Export</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showClear && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, backgroundColor: 'var(--q-overlay)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowClear(false)}>
          <div style={{ backgroundColor: 'var(--q-bg-elevated)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-modal)', animation: 'modalEnter 250ms cubic-bezier(0.16, 1, 0.3, 1)', padding: '24px', maxWidth: '400px', width: '90%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
              <button onClick={() => setShowClear(false)} onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.06)' }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent' }} style={{ padding: '7px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--q-border)', cursor: 'pointer', backgroundColor: 'transparent', color: 'var(--q-accent-danger)', fontSize: '13px', fontFamily: 'var(--font-interface)' }}>Cancel</button>
              <div style={{ width: '8px' }} />
