...
const unsubDebugLog = subscribe('debug_log', (p: any) => {
  ...
  const selectSession = useCallback(async (sessionKey: string) => {
    ...
    // I badge si azzerano istantaneamente, niente scroll strani, niente marker fantasma. ===
      ...
      if (history?.messages) {
        ...
        for (const m of history.messages) {
          if (m.role === 'delegation') {
            ...
            // Add to last assistant message's blocks (at current position)
